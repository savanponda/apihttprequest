...
const updateBoook = async (req, res) => {
    try {
      ...
      db.query('UPDATE book SET title = ?, description = ?, published_year = ?, quantity_available = ?, author_id = ?, genre_id = ? WHERE book_id = ?', [title, description, published_year, quantity_available, author_id, genre_id,bookId], (error, results, fields) => {
        if (error) {
          console.log(error);
